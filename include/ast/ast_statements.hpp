...
public:
    ...
    void generateMIPS(CompContext &context, std::vector<Instruction> &instructions, char destReg = 0) {
        ...
        for (int i = 0; i < (int)context.switchFlags().caseFlags.size(); ++i) {
            ...
            instructions.push_back({"addi", regMap[caseReg], regMap[expReg], "", -context.switchFlags().caseFlags[i].second, Instruction::SSN});
            instructions.push_back({"beq", regMap[expReg], regMap[$zero], context.switchFlags().caseFlags[i].first, 0, Instruction::SSS});
        }
